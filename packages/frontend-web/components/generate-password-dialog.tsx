...
export function GeneratePasswordDialog({ onClose }: GeneratePasswordDialogProps) {
  ...
  const getStrengthLabel = () => {
    if (passwordStrength < 30)
      return { label: "Weak", icon: ShieldAlert, color: "text-red-500", bgColor: "bg-red-100 dark:bg-red-900/30" }
    if (passwordStrength < 60)
      return { label: "Fair", icon: Shield, color: "text-yellow-500", bgColor: "bg-yellow-100 dark:bg-yellow-900/30" }
    if (passwordStrength < 80)
      return { label: "Good", icon: Shield, color: "text-blue-500", bgColor: "bg-blue-100 dark:bg-blue-900/30" }
    return { label: "Strong", icon: ShieldCheck, color: "text-green-500", bgColor: "bg-green-100 dark:bg-green-900/30" }
  }
      ...
      <div className="w-full max-w-2xl rounded-lg bg-card p-6 border border-border shadow-lg">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            >
              <CardContent className="p-3 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-xs text-muted-foreground">20 chars</span>
              </CardContent>
            </Card>
            <Card
              className="cursor-pointer hover:border-primary transition-colors"
            ...
            >
              <CardContent className="p-3 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-xs text-muted-foreground">14 chars</span>
              </CardContent>
            </Card>
            ...
            >
              <CardContent className="p-3 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-xs text-muted-foreground">18 chars</span>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="relative">
            ...
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">{passwordStrength}% Strength</span>
              </div>
            ...
            <TabsContent value="options" className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-muted/30 p-4 rounded-md">
                  <h3 className="text-sm font-medium mb-2">Password Analysis</h3>
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-center gap-2">
                      ...
                      <span className={passwordStrength > 60 ? "" : "text-muted-foreground"}>
                        {passwordStrength > 60
                          ? "Strong enough for most sites"
                          : "Not strong enough for sensitive accounts"}
                      </span>
                    </li>
                    <li className="flex items-center gap-2">
                      <div className={`h-2 w-2 rounded-full ${length >= 12 ? "bg-green-500" : "bg-gray-300"}`}></div>
                      <span className={length >= 12 ? "" : "text-muted-foreground"}>
                        {length >= 12 ? "Good length" : "Consider using a longer password"}
                      </span>
                    </li>
                    <li className="flex items-center gap-2">
                      <div
                      ...
                      >
                        {options.uppercase && options.lowercase && options.numbers && options.symbols
                          ? "Using all character types"
                          : "Consider using more character types"}
                      </span>
                    </li>
                  </ul>
                </div>
            ...
            <TabsContent value="advanced" className="space-y-4 mt-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label className="text-sm cursor-pointer" onClick={() => handleOptionChange("easyToRead")}>
                    Easy to Read (no similar characters like 1, l, I, 0, O)
                  </label>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label className="text-sm cursor-pointer" onClick={() => handleOptionChange("easyToSay")}>
                    Easy to Say (no numbers or special characters)
                  </label>
                </div>
              </div>
              ...
              <div className="mt-4 p-4 bg-muted/50 rounded-md">
                ...
                <ul className="text-sm space-y-2 text-muted-foreground">
                  <li>• Use a minimum of 14 characters for high security</li>
              ...
              <div className="space-y-2">
                ...
                {passwordHistory.length > 0 ? (
                  ...
                  <div className="p-8 text-center text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">Generated passwords will appear here</p>
                  </div>
                )}
              </div>
            </TabsContent>
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button onClick={copyToClipboard}>{copied ? "Copied!" : "Copy Password"}</Button>
          </div>
        </div>
      </div>
    </div>
