...
export function EditCardDialog({
  ...
  // Initialize form with card data when it changes
  useEffect(() => {
    if (card) {
      setTitle(card.title || "");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axiosInstance.put(
        `/${selectedWorkspaceId}/${selectedProjectId}/cards/${card.doc_id}`,
        payload
      );

      toast({
        title: translate("success", "actions"),
        description: translate("card_updated_successfully", "cards", { default: "Card updated successfully" }),
      });
      ...
      let errorMessage = translate("error_updating_card", "cards", { default: "Error updating card" });
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button type="submit" onClick={handleSubmit} disabled={isSubmitting}>
            {isSubmitting
              ? translate("updating", "cards", { default: "Updating..." })
