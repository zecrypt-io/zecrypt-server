...
from app.api.v1.web.drive.folders.services import create_folder, get_root_folders, get_folder_by_id, delete_folder, rename_folder
...
async def create_folder_api(payload: Folder, user: UserDetails = Depends(get_current_user)):
    return await create_folder(user.get("db"), user.get("user_id"), payload.model_dump())
...
async def get_folders_api(
    ...
    return await get_root_folders(user.get("db"), user.get("user_id"), parent_id)
...
async def get_folder_by_id_api(
    ...
    return await get_folder_by_id(user.get("db"), user.get("user_id"), folder_id)
...
async def delete_folder_api(
    ...
    return await delete_folder(user.get("db"), user.get("user_id"), folder_id, background_task)

@router.post(FOLDER_URL+"/rename")
async def rename_folder_api(payload: RenameFolder, user: UserDetails = Depends(get_current_user)):
    return await rename_folder(user.get("db"), user.get("user_id"), payload.model_dump())