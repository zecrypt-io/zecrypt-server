...
def permission_check(
    required_permissions, project_id, user_id, company_db, permission_type="access"
):
    project = project_repo.find_one(
        company_db, {"project_id": project_id, "access": {"$ne": False}}
    )
    if not project:
        logger.warning("Project not found")
        raise HTTPException(status_code=404, detail=_("Project not found"))
    admin_user = check_if_admin_user(user_id)
    stages = [
    ...
    for member in project.get("members"):
        if member.get("user_id") == user_id:
            if permission_type == "access":
                if member.get("permission") in required_permissions:
                    return {
                        "permission": member.get("permission"),
                        "stages": member.get("stages"),
                        "status": project.get("status"),
                    }
